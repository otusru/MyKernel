...
1. Загрузчик (Bootloader). Ассемблерный код загрузчика и конфигурация GRUB.
...
2. Ядро (Kernel). Реализация ядра, включая обработку прерываний, планировщик и драйверы.
...
3. Библиотеки (lib). Библиотеки для строковых операций и управления памятью.
string.c: Реализация стандартных строковых функций.
memory.c: Функции для работы с памятью, такие как memcpy, memset.

4. Заголовочные файлы (include). Заголовочные файлы для ядра.
...
5. Cборка (Makefile). Файл сборки проекта.
